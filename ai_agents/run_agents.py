...
def _default_time_range_last_30m() -> tuple[int, int, str]:
...
async def main() -> None:
        ...
        if resp.status_code == 200:
            ...
            if ids:
                if requested_model not in ids:
                    selected_model = ids[0]
                    print(
                        f"[Info] Your OLLAMA_MODEL='{requested_model}' was not found in Ollama; "
                        f"falling back to the first available model: '{selected_model}'"
                    )
            else:
                print("[Info] Unable to read /v1/models (empty model list).")
        else:
            print(
                f"[Info] Unable to read /v1/models (HTTP {resp.status_code}); "
                f"will try to use OLLAMA_MODEL='{requested_model}' directly."
            )
    except Exception:
    ...
    from_ts, to_ts, window_label = _default_time_range_last_30m()
    ...
    pm = AssistantAgent(
        ...
        system_message=(
            ...
            "  then respond directly in English describing the app's capabilities in 2–5 sentences and end with TERMINATE.\n"
    ...
    def _extract_consensus(content: str) -> tuple[int | None, str | None]:
        
        import re
        m = re.search(r"CONSENSUS\\s+ROUND\\s*=\\s*(\\d+)\\s+STATUS\\s*=\\s*(AGREE|DISAGREE)", content, re.IGNORECASE)
        if not m:
            return None, None
        return int(m.group(1)), m.group(2).upper()

    def _last_round_status(messages, source: str) -> tuple[int | None, str | None]:
        for msg in reversed(messages):
            ...
            if not isinstance(content, str):
                continue
            return _extract_consensus(content)
        return None, None

    def _last_status(messages, source: str) -> str | None:
        r, s = _last_round_status(messages, source)
        return s

    def _count_summary_msgs(messages) -> int:
        c = 0
        for msg in messages:
            if getattr(msg, "source", None) in ("Summary", "Summary2"):
                c += 1
        return c
    ...
    async def selector_func(messages) -> str | None:
        ...
        summary_count = _count_summary_msgs(messages)
        a_status = _last_status(messages, "Summary")
        b_status = _last_status(messages, "Summary2")
        max_debate_msgs = 8
        ...
        if summary_count >= 4 and a_status == "AGREE" and b_status == "AGREE":
            return "Advice"
    ...
    default_task = "Analyze disk usage over the default time window. Is there any risk of running out of space?"
    env_task = os.getenv("USER_TASK")
    if env_task:
        user_msg = env_task.strip()
    else:
        try:
            user_msg = input(
                "Enter a question to analyze (e.g., 'Check whether disk usage is close to full'):\n> "
            ).strip()
        except EOFError:
            user_msg = default_task
    if not user_msg:
        user_msg = default_task
    ...
    try:
        ...
        print(
            "\n[Error] Failed to start the agent pipeline. Common causes: Ollama is not running or the model name is invalid."
        )
        print("You can run `ollama list` and set the desired model name in OLLAMA_MODEL.")
        print(f"Details: {e}")
        return
    ...
    for m in result.messages:
        ...
        if src in ("Summary", "Summary2"):
            round_no = (sum_idx // 2) + 1
            print(f"\n[Debate Round {round_no}][{src}] {m.content}")
            sum_idx += 1
        else:
            print(f"\n[{src}] {m.content}")
